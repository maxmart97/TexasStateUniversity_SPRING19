$(function() { // START of $(document).ready()
    ...
    $("#add-order").click(function() {
        if (validateInput()) {
            ...
            $.ajax({
                type: "POST",
                url: "http://rest.learncode.academy/api/1234567890cis33600987654321mm/ajax-practice-a2",
                //dataType: "json",
                data: order,
                success: function() {
        ...
        if (validateInput()) {
            ...
            if (orderID === "") {
                ...
                if ($orders.children("li").length > 0) {
                    $.ajax({
                        type: "PUT",
                        url: "http://rest.learncode.academy/api/1234567890cis33600987654321mm/ajax-practice-a2/" + orderID,
                        data: order,
                        success: function() {
                            updateOrderList();
                            resetTable();
